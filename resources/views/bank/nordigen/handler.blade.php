...
<script>
    ...
    const config = {
        ...
        text: "{{ (isset($company) && !is_null($company) ? $company->name : '') }} {{ ctrans('texts.nordigen_handler_subtitle', [], $lang ?? 'en') }}",
        // Logo URL that will be shown below the modal form.
        logoUrl: "{{ (isset($company) && !is_null($company) ? $company->present()->logo() : '') }}",
