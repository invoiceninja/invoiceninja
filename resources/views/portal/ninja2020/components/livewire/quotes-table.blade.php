<div>
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center">
            <div class="mr-3">
                <input wire:change="toggleStatus('{{ App\Models\Quote::STATUS_SENT }}')" @checked(in_array(App\Models\Quote::STATUS_SENT, $status)) value="{{ App\Models\Quote::STATUS_SENT }}" type="checkbox" class="cursor-pointer form-checkbox" id="sent-checkbox">
                <label for="sent-checkbox" class="text-sm cursor-pointer">{{ ctrans('texts.status_pending') }}</label>
            </div>
            <div class="mr-3">
                <input wire:change="toggleStatus('{{ App\Models\Quote::STATUS_APPROVED }}')" @checked(in_array(App\Models\Quote::STATUS_APPROVED, $status)) value="{{ App\Models\Quote::STATUS_APPROVED }}" type="checkbox" class="cursor-pointer form-checkbox" id="approved-checkbox">
                <label for="approved-checkbox" class="text-sm cursor-pointer">{{ ctrans('texts.approved') }}</label>
            </div>
            <div class="mr-3">
                <input wire:change="toggleStatus('{{ App\Models\Quote::STATUS_EXPIRED }}')" @checked(in_array(App\Models\Quote::STATUS_EXPIRED, $status)) value="{{ App\Models\Quote::STATUS_EXPIRED }}" type="checkbox" class="cursor-pointer form-checkbox" id="expired-checkbox">
                <label for="expired-checkbox" class="text-sm cursor-pointer">{{ ctrans('texts.expired') }}</label>
            </div>
            <div class="mr-3">
                <input wire:change="toggleStatus('{{ App\Models\Quote::STATUS_REJECTED }}')" @checked(in_array(App\Models\Quote::STATUS_REJECTED, $status)) value="{{ App\Models\Quote::STATUS_REJECTED }}" type="checkbox" class="cursor-pointer form-checkbox" id="rejected-checkbox">
                    ...
                    @forelse($quotes as $quote)
                        <tr class="bg-white group hover:bg-gray-100" wire:key="quote-{{ $quote->hashed_id }}">
                            <td class="px-6 py-4 text-sm font-medium leading-5 text-gray-900 whitespace-nowrap">
                                <label>
                                    <input type="checkbox" class="form-check" wire:change="toggleSelected('{{ $quote->hashed_id }}')" @checked(in_array($quote->hashed_id, $selected, true)) value="{{ $quote->hashed_id }}">
                                </label>
                            </td>
