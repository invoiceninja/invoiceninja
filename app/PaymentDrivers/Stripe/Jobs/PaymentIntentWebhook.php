...
class PaymentIntentWebhook implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // nlog(optional($this->stripe_request['data']['object']['charges']['data'][0]['metadata'])['gateway_type_id']);
        // nlog(optional($this->stripe_request['data']['object']['charges']['data'][0]['metadata'])['payment_hash']);
        // nlog(optional($this->stripe_request['data']['object']['charges']['data'][0]['payment_method_details']['card'])['brand']);

        MultiDB::findAndSetDbByCompanyKey($this->company_key);

        if(optional($this->stripe_request['data']['object']['charges']['data'][0])['id']){

            $company = Company::where('company_key', $this->company_key)->first();

            $payment = Payment::query()
                             ->where('company_id', $company->id)
                             ->where('transaction_reference', $this->stripe_request['data']['object']['charges']['data'][0]['id'])
                             ->first();
