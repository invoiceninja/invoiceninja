...
class ACH implements LivewireMethodInterface
{
    ...
    public function paymentView(array $data)
    {
        ...
        return render('gateways.lawpay.ach.pay', $data);
    }
    ...
    public function paymentResponse(PaymentResponseRequest $request)
    {
        ...
        // Handle payment with saved token
        ...
        // Store payment method + token billing if configured
        if (isset($this->lawpay->company_gateway->token_billing) && $this->lawpay->company_gateway->token_billing != 'off') {
            $payment_meta = new \stdClass();
            $payment_meta->exp_month = '';
            $payment_meta->exp_year = '';
            $payment_meta->brand = 'ACH';
            ...
            $cgt = $this->lawpay->storeGatewayToken($token_data, []);
        ...
        // One-time charge with token from hosted fields
        ...
        if ($response->successful()) {
            ...
            // ACH is asynchronous — payment starts as pending until webhook confirms
            $payment = $this->lawpay->createPayment($data, Payment::STATUS_PENDING);
    ...
    public function tokenBilling(ClientGatewayToken $cgt, PaymentHash $payment_hash): Payment
    {
        ...
        $error_message = $lawpay_response['message'] ?? $lawpay_response['error'] ?? 'Payment failed';

        SystemLogger::dispatch(
            ['server_response' => $lawpay_response, 'data' => $payload],
            SystemLog::CATEGORY_GATEWAY_RESPONSE,
            SystemLog::EVENT_GATEWAY_FAILURE,
            LawPayPaymentDriver::SYSTEM_LOG_TYPE,
            $this->lawpay->client,
    ...
    {
        return 'gateways.lawpay.ach.pay_livewire';
    }
