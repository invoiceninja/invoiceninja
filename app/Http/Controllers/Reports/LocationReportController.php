...
class LocationReportController extends BaseController
{
    ...
    /**
     * @OA\Post(
     *      path="/api/v1/reports/locations",
     *      operationId="getLocationReport",
     *      tags={"reports"},
     *      summary="Location reports",
     *      description="Export location reports",
    ...
    public function __invoke(GenericReportRequest $request)
    {
        ...
        if ($request->has('send_email') && filter_var($request->input('send_email'), FILTER_VALIDATE_BOOLEAN) && $request->missing('output')) {
            SendToAdmin::dispatch($user->company(), $request->all(), LocationExport::class, $this->filename);
        ...
        PreviewReport::dispatch($user->company(), $request->all(), LocationExport::class, $hash, $this->filename, $user);
